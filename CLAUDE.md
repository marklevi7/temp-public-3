...
2. Publish `ptt-wireframe.html` with the `Artifact` tool (same file path each time so the URL stays stable).
3. Reply with the artifact URL so it renders in the app's side panel.
4. ALWAYS include the githack link too, every single time — it is the only one where the microphone works:
...
Do NOT send the file with `SendUserFile` (iOS Quick Look doesn't run JS).
The Browser-pane preview is the live in-session view; the artifact link is the in-app preview; the githack link is for real mic testing. Do all three.
